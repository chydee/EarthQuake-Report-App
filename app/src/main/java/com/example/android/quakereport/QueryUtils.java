...
public class QueryUtils {
    ...
    public static ArrayList<EarthQuake> extractEarthquakes() {
        ...
        try {
            ...
            //Get the instance of the JSONArray that contains JSONObject
            JSONArray jsonArray = jsonRootObject.optJSONArray("features");

            for (int i = 0; i < jsonArray.length(); i++) {
                JSONObject jsonObject = jsonArray.optJSONObject(i);
                JSONObject newJSONObj = jsonObject.optJSONObject("properties");
                double magnitude = newJSONObj.optDouble("mag");
                String location = newJSONObj.optString("place");
                long time = newJSONObj.optLong("time");
                String url = newJSONObj.optString("url");
